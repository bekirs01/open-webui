...
# Bash 3.2 uyumlu küçük harf (${VAR,,} macOS /bin/bash 3.2'de yok)
ow_lower() {
	printf '%s' "${1:-}" | tr '[:upper:]' '[:lower:]'
}

# Add conditional Playwright browser installation
if [[ "$(ow_lower "$WEB_LOADER_ENGINE")" == "playwright" ]]; then
...
if [[ "$(ow_lower "$USE_OLLAMA_DOCKER")" == "true" ]]; then
    echo "USE_OLLAMA is set to true, starting ollama serve."
    ollama serve &
fi

if [[ "$(ow_lower "$USE_CUDA_DOCKER")" == "true" ]]; then
