	...
	>
		...
		<div class="w-full group">
			<div
				...
				class="relative w-full py-1 px-1.5 rounded-xl flex items-center gap-1.5 hover:bg-gray-100 dark:hover:bg-neutral-800 transition {$selectedFolder?.id ===
				folderId
			...
			>
				<button
					class="text-gray-500 dark:text-gray-500 transition-all p-1 hover:bg-gray-200 dark:hover:bg-neutral-800 rounded-lg"
					on:click={(e) => {
						e.stopPropagation();
				...
				>
					...
					>
						<div class="p-1 dark:hover:bg-neutral-800 rounded-lg touch-auto">
		...
		<div slot="content" class="w-full">
			{#if (folders[folderId]?.childrenIds ?? []).length > 0 || (chats ?? []).length > 0}
				<div
					class="ml-3 pl-1 mt-[1px] flex flex-col overflow-y-auto scrollbar-hidden border-s border-gray-100 dark:border-neutral-900"
				>
