<script lang="ts">
	...
	const entry = NODE_REGISTRY.httpRequest;
	...
	$: urlShort = (data.url ?? '').replace(/^https?:\/\//, '').slice(0, 36);
</script>

<div
	role="group"
	on:dblclick|stopPropagation={() => openNodeInspector(id)}
	class="relative rounded-lg border shadow-sm w-[196px] max-w-[92vw] bg-teal-50/90 dark:bg-teal-950/50 transition-shadow {isDisabled
...
>
	{#if runActive}
		...
		/>
	{/if}
	<div class="flex items-center gap-1.5 px-2 py-1.5 border-b border-teal-100 dark:border-teal-900">
		<span class="text-sm leading-none" aria-hidden="true">{entry.icon}</span>
		<span class="text-[11px] font-semibold text-teal-900 dark:text-teal-100 flex-1 truncate">
	...
	<div class="px-2 py-1 text-[9px] text-teal-900/85 dark:text-teal-200/85 truncate" title={data.url}>
		<span class="font-mono text-gray-600 dark:text-gray-400">{methodVal}</span>
		{' '}
		{urlShort || '…'}
	</div>
	<Handle
