<script lang="ts">
	...
	const entry = NODE_REGISTRY.trigger;
...
<div
	...
	class="relative rounded-lg border shadow-sm w-[196px] max-w-[90vw] bg-amber-50/90 dark:bg-amber-950/50 transition-shadow {isDisabled
...
>
	...
	{/if}
	<div
		class="flex items-center gap-1.5 px-2 py-1.5 border-b border-amber-100 dark:border-amber-900"
	>
		<span class="text-sm leading-none" aria-hidden="true">{entry.icon}</span>
		<span class="text-[11px] font-semibold text-amber-900 dark:text-amber-100 flex-1 truncate">
	...
	<div class="px-2 py-1 text-[9px] text-amber-900/80 dark:text-amber-200/80 truncate" title={data.label}>
		{data.label || $i18n.t('Passes user message')}
	</div>
	<Handle
		type="source"
		position={Position.Right}
		class="!w-2.5 !h-2.5 !bg-amber-600 !border-0"
	/>
</div>
