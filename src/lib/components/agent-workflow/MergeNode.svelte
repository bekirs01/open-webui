<script lang="ts">
	...
	const entry = NODE_REGISTRY.merge;

	$: isStart = id === $startNodeId;
	$: runActive = $runActiveNodeId === id;
	$: runPath = $runPathNodeIds.has(id);
	$: isDisabled = Boolean(data.disabled);
	$: sepPreview = (data.separator ?? '').replace(/\n/g, '↵').slice(0, 28);
</script>

<div
	role="group"
	on:dblclick|stopPropagation={() => openNodeInspector(id)}
	class="relative rounded-lg border shadow-sm w-[196px] max-w-[90vw] bg-violet-50/90 dark:bg-violet-950/50 transition-shadow {isDisabled
...
>
	...
	<div class="px-2 py-1 text-[9px] text-violet-800/90 dark:text-violet-200/90 truncate" title={data.separator}>
		{sepPreview || '—'}
	</div>
	<Handle
		type="target"
