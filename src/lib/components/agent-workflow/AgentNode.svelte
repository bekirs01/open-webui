<script lang="ts">
	...
	const entry = NODE_REGISTRY.agent;

	$: isStart = id === $startNodeId;
	$: runActive = $runActiveNodeId === id;
	$: runPath = $runPathNodeIds.has(id);
	$: isDisabled = Boolean(data.disabled);
	$: subtitle = `${(data.agentName || '').trim() || $i18n.t('Agent')} · ${data.mode}`;
</script>

<div
	role="group"
	on:dblclick|stopPropagation={() => openNodeInspector(id)}
	class="relative rounded-lg border shadow-sm w-[196px] max-w-[90vw] bg-white dark:bg-gray-900 transition-shadow {isDisabled
...
>
	{#if runActive}
		...
		/>
	{/if}
	<div class="flex items-center gap-1.5 px-2 py-1.5 border-b border-gray-100 dark:border-gray-800">
		<span class="text-sm leading-none" aria-hidden="true">{entry.icon}</span>
		<span class="text-[11px] font-semibold text-gray-800 dark:text-gray-100 flex-1 truncate">
	...
	<div class="px-2 py-1 text-[9px] text-gray-600 dark:text-gray-400 truncate" title={subtitle}>
		{subtitle}
	</div>
	<Handle
		type="target"
