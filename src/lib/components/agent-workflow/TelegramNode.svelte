<script lang="ts">
	...
	const entry = NODE_REGISTRY.telegram;

	$: isStart = id === $startNodeId;
	$: runActive = $runActiveNodeId === id;
	$: runPath = $runPathNodeIds.has(id);
	$: isDisabled = Boolean(data.disabled);
	$: mode = data.credentialMode === 'inline' ? 'inline' : 'env';
	$: chatShort = (data.chatId ?? '').slice(0, 28);
</script>

<div
	role="group"
	on:dblclick|stopPropagation={() => openNodeInspector(id)}
	class="relative rounded-lg border shadow-sm w-[196px] max-w-[92vw] bg-sky-50/90 dark:bg-sky-950/50 transition-shadow {isDisabled
...
>
	{#if runActive}
		<div
			class="absolute right-1.5 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-sky-300 border-t-transparent animate-spin dark:border-sky-500 dark:border-t-transparent"
			aria-hidden="true"
		/>
	{/if}
	...
	<div class="px-2 py-1 text-[9px] text-sky-900/85 dark:text-sky-200/85 truncate" title={data.chatId}>
		<span class="font-mono text-gray-600 dark:text-gray-400">{mode}</span>
		{' '}
		{chatShort || 'chat…'}
	</div>
	<Handle
